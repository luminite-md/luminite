...
- 访问[官方网站](https://luminite.md)下载最新 macOS 版本。
- 将 `Luminite.app` 拖入**应用程序**文件夹。

### 2. 初始配置

1. 启动 Luminite，完成引导设置。
2. 配置 LLM 提供商（使用预设 API 或自带 Key）。
3. 连接数据源——本地文件夹、Notion、飞书、Apple Notes 等。

### 3. 开始使用

- **提问检索**：打开对话面板，跨全部已连接知识源进行提问。
- **导入文档**：将 PDF、Markdown、Word、PPT 等文件拖入 Luminite，自动完成索引。
- **沉淀知识**：一键将对话中的深度洞察持久化到本地知识库。

### 4. MCP 集成（可选）

启用内置 MCP Server，让外部 AI 工具（如 Claude、Cursor）直接访问你的本地知识网络。
